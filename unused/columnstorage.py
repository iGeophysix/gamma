...
class ColumnStorage:
    ...
    @my_timer
    def bulk_load_dataset(self, wellname, datasetname, logs: dict, values: dict, size: int = 8192, trunc=True, autocommit=True) -> None:
        with self._engine.cursor() as cursor:
            if trunc:
                try:
                    self.delete_dataset(wellname, datasetname)
                finally:
                    self.create_dataset(wellname, datasetname)

            for log, dtype in logs.items():
                self.add_log(wellname, datasetname, log, self.get_data_type(dtype), autocommit=False)
            self.commit()
            ...
            cursor.copy_from(data_to_insert, f'"{self.__generate_dataset_name(wellname, datasetname)}"', sep='|', size=size, null=str(MISSING_VALUE))
    ...
    def read_dataset(self, wellname, datasetname, logs=None, depth=None, depth__gt=None, depth__lt=None):
        if logs:
            ...
            fields = '*'
        query = f'select {fields} from "{self.__generate_dataset_name(wellname, datasetname)}" where 1=1'
        if depth:
    ...
    @my_timer
    def update_dataset(self, wellname, datasetname, data, autocommit=True):
        ...
        query = f"""
        insert into "{self.__generate_dataset_name(wellname, datasetname)}" (reference, {logs}) 
        values 
        ({values})
    ...
    def add_log(self, wellname, datasetname, log_name, log_type, autocommit=True):
        query = sql.SQL(f"""ALTER TABLE "{self.__generate_dataset_name(wellname, datasetname)}" ADD COLUMN "{log_name}" {log_type} NULL;""")
        self._conn.execute(query)
        if autocommit:
            self._engine.commit()
        return log_name

    def delete_log(self, wellname, datasetname, log_name, autocommit=True):
        query = sql.SQL(f"""ALTER TABLE "{self.__generate_dataset_name(wellname, datasetname)} DROP COLUMN {log_name}" ;""")
        self._conn.execute(query)
